...
function renderComments(thread, container) {
  ...
  // This line displays likes, reposts, and replies (you can remove or keep this as needed)
  ...
  // Display only the first 10 comments/replies (or fewer if there are less than 10)
  if (thread.replies && thread.replies.length > 0) {
    const commentsContainer = document.createElement("div");

    commentsContainer.id = "comments-container";

    // Limit to the first 10 replies
    const limitedReplies = thread.replies.slice(0, 15);
    const sortedReplies = limitedReplies.sort(sortByLikes);
...
function renderComment(comment) {
  ...
  authorLink.textContent = author.displayName ?? author.handle; // This shows the name of the replier

  authorDiv.appendChild(authorLink);

  // Removed the handle part here so that only the name is displayed
